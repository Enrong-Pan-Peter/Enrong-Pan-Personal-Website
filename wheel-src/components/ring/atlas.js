...
// Portrait cells, 2:3, matching the plane. Images are contain-fit so nothing
// is cropped; the margins are painted in the page background colour.
const CELL_W = 512;
const CELL_H = Math.round(CELL_W * 1.5);
const CELL_BG = "#fafafa";
...
export function buildAtlas(files = IMAGE_FILES, onProgress) {
  ...
  const paint = (img, i) => {
    ...
    // Contain fit: show the whole photo in its own orientation, no cropping.
